...
# Connect UnifAPI to Cline

> Paste one prompt · read-only public-data MCP over OAuth — **eyes, not hands**.

Connect Cline to the UnifAPI public-data MCP server and run marketing research — SEO, AI-visibility (GEO), social, local, and competitive intelligence — from live public data without leaving Cline. Paste one prompt and the agent installs the MCP server over OAuth, or add the hosted endpoint by hand. One OAuth sign-in, no API key to paste; everything is read-only.

## Steps

### 1. Paste one prompt into Cline

Drop this into Cline and let it run — it adds the server and starts the OAuth sign-in for you:
...
Add UnifAPI to Cline's MCP config:
...
<sub>Topics: Cline MCP · connect UnifAPI to Cline · Cline MCP server · UnifAPI Cline setup · Cline public data API</sub>
